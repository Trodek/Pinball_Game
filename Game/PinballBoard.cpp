...
bool PinballBoard::Update(float dt)
{
	...
	if (bonus_rb && bonus_lo && bonus_rn && bonus_ru && bonus_rs) {
		AddBonusScore();
		bonus_rb = bonus_ro = bonus_rn = bonus_ru = bonus_rs = false;
	}
...
void PinballBoard::OnCollision(PhysBody * bodyA, PhysBody * bodyB)
{
	if (bodyA == launch_triger) {
	...
	else if (bodyA == rectangle1 || bodyA == rectangle2 || bodyA == rectangle3 || bodyA == rectangle4 || bodyA == rectangle5 || bodyA == rectangle6 || bodyA == rectangle7 || bodyA == rectangle8
		|| bodyA == rectangle9 || bodyA == rectangle10 || bodyA == rectangle11 || bodyA == rectangle12 || bodyA == rectangle13 || bodyA == rectangle14 || bodyA == rectangle15 || bodyA == rectangle16
		...
		if (bodyB == ball) {
			...
			if (bodyA == rectangle2 || bodyA == rectangle7) {
				pinkrect2_collided = true;
				pinkrect_time_start = count;
			}
			if (bodyA == rectangle4 || bodyA == rectangle6) {
...
bool PinballBoard::CreateBouncers()
{
	...
	rectangle14 = App->physics->CreateStaticRectangle(252, 354, 3, 15, 1.025f, BOARD, BALL, -45);
	rectangle14->listener = App->pinball;
	rectangle15 = App->physics->CreateStaticRectangle(155, 280, 3, 15, 1.025f, BOARD, BALL, -30);
